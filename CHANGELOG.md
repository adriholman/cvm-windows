...
### Fixed
- Download logic now attempts to load `System.Net.Http` explicitly and falls back to `Invoke-WebRequest` when `System.Net.Http.HttpClientHandler` is unavailable in the current PowerShell environment
- Prevented repeated retry failures during `cvm install stable` caused by missing `HttpClientHandler` type resolution
...
### Fixed
- cvm now validates and rejects unknown command-line flags to prevent passing invalid arguments to Composer
- Added strict validation for supported cvm commands, preventing confusing Symfony Console exceptions
